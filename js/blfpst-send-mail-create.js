...
$jq(document).ready(function () {
    ...
    if ($jq("#test_targets")[0]) {
        if (!isReserved()) {
            $jq('#change_reserved').removeClass('btn-success');
            $jq('#change_reserved').addClass('btn-default');
        ...
        $jq('#mainForm').parsley().on('field:validated', function () {
            ...
            if (container) {
                ...
                var $iconObj = $containerObj.find('i[data-blfpst-icon-for="' + name + '"]');
...
function isReserved() {
    ...
    var reserved = $jq('input[name=send_type]').val();
...
function changeReserved() {
    $jq('#change_reserved').toggleClass('btn-success');
    $jq('#change_reserved').toggleClass('btn-default');

    var reservedPanel = $jq('#reserved_panel');
    var notReservedPanel = $jq('#not_reserved_panel');

    if (!isReserved()) {
        $jq('input[name=send_type]').val('reserved');
        reservedPanel.show();
        notReservedPanel.hide();
    } else {
        $jq('input[name=send_type]').val('');
        reservedPanel.hide();
...
function selectTarget(n) {
    ...
    if (target_id > 0) {
        ...
        $jq('#target_name_button').addClass('btn-default');
    } else {
        $jq('#target_name_button').removeClass("btn-default");
