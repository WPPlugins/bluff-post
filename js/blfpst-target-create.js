...
function conditionalLine(param_name, new_param, group_count) {
    ...
    if (new_param > 0) {
        ...
        delete_command = ' <a href="javascript:void(0)" onclick="deleteConditional(' + group_count + ', ' + new_param + ')" id="delete' + param_name + '"><i class="fa fa-minus-circle" aria-hidden="true"></i></a>';
    } else {
...
function addGroup() {
    ...
    if (group_count == 0) {
        ...
        html += '<select name="and_or' + new_group + '">' +
            '<option value="AND">AND</option>' +
            '<option value="OR">OR</option>' +
            '</select>';
        delete_command = ' <a href="javascript:void(0)" onclick="deleteGroup(' + group_count + ')" id="delete' + group_count + '"><i class="fa fa-minus-circle" aria-hidden="true"></i></a>';
    }

    html += '<div class="panel panel-default" id="group' + new_group + '">' +
        ...
        '<button type="button" class="btn btn-default btn-xs" id="addConditional' + new_group + '" onclick="addConditional(' + new_group + ')">' +
...
function build_recipients_preview(recipients, page_num, limit, total_page, total_count) {
    ...
    if (total_page > 1) {
        ...
        var $li = $jq('<li>').appendTo($pagenation);
        $li.append('<a href="javascript:void(0)" aria-label="Previous" onclick="onRecipientsPreviewButton(' + pre_page + ', ' + limit + ')"> <span aria-hidden="true">&laquo;</span> </a>');

        for (i = start_page; i < stop_page; i++) {
            if (page_num == i) {
                $li = $jq('<li class="active">').appendTo($pagenation);
            } else {
                $li = $jq('<li>').appendTo($pagenation);
            }

            $li.append('<a href="javascript:void(0)" onclick="onRecipientsPreviewButton(' + i + ', ' + limit + ')">' + (i + 1) + '</a>');
        }

        $li = $jq('<li>').appendTo($pagenation);
        $li.append('<a href="javascript:void(0)" aria-label="Next" onclick="onRecipientsPreviewButton(' + next_page + ', ' + limit + ')"> <span aria-hidden="true">&raquo;</span> </a>');
    }
}
