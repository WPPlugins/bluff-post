...
<div class="container">
	<div class="page-header">
		<h1><?php esc_html_e( 'Trash', 'bluff-post' ) ?></h1>
	</div>
	<?php if ( ! empty( $errors ) ) : ?>
								...
								<tr>
									...
									<td>
										<button type="button" class="btn btn-default btn-xs"
										        onclick="recycleMail(<?php echo esc_html( $send_mail_id ) ?>)">
					...
					<ul class="pagination">
						...
						for ( $i = $start_page; $i < $stop_page; $i ++ ) {
							...
							<li <?php if ( $page_num == $i ) { ?>class="active"<?php } ?>>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-trashes&page_num=' . $i ) ) ?>"><?php echo( $i + 1 ) ?></a>
							</li>
							<?php
						}
						?>
