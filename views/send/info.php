...
<div class="container">
	<div class="page-header">
		<h1><?php echo esc_html( stripslashes( $subject ) ) ?></h1>

		<div class="row">
			<div class="col-sm-6">
				<?php if ( $is_reserved ) : ?>
					<i class="fa fa-bell" aria-hidden="true"></i> <?php echo esc_html( $reserved_at ) ?>
				<?php endif ?>
			</div>
		</div>
		...
		<?php if ( ! empty( $send_request_start_at ) ) : ?>
			<div class="row">
				<div class="col-sm-6">
					<?php if ( ! empty( $send_request_end_at ) ) : ?>
		...
		<div class="row">
			<div class="col-sm-6">
				<i class="fa fa-reply" aria-hidden="true"></i> <?php echo esc_html( stripslashes( $reply_address ) ) ?>
			</div>
		</div>
	</div>
	...
	<div class="row outer_block">
		<div class="col-sm-8 col-sm-offset-2">
			<div class="col-sm-4 text-center">
				<div><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Destination count', 'bluff-post' ) ?>">
						<i class="fa fa-child fa-5x" aria-hidden="true"></i>
					</span></div>
				<div style="font-size: 32px;"><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Destination count', 'bluff-post' ) ?>">
			...
			<div class="col-sm-4 text-center">
				<div><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Transmission count', 'bluff-post' ) ?>">
						<i class="fa fa-motorcycle fa-5x" aria-hidden="true"></i>
					</span></div>
				<div style="font-size: 32px;"><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Transmission count', 'bluff-post' ) ?>">
						<?php echo esc_html( number_format( $success_count ) ) ?>
					</span></div>
			</div>
			<div class="col-sm-4 text-center">
				<div><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Failure count', 'bluff-post' ) ?>">
						<i class="fa fa-minus-circle fa-5x" aria-hidden="true"></i>
					</span></div>
				<div style="font-size: 32px;"><span data-toggle="tooltip" data-placement="right" title="<?php esc_html_e( 'Failure count', 'bluff-post' ) ?>">
					...
					<div class="alert alert-danger" role="alert">
						<ul>
							<?php foreach ( $error_logs as $error_log ) : ?>
								<li>
									<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <strong><?php echo esc_html( $error_log->summary ) ?></strong> <?php echo esc_html( $error_log->detail ) ?>
								</li>
							<?php endforeach ?>
						</ul>
					</div>
	...
	<?php if ( $is_sending || $is_history ) : ?>
		...
		<?php if ( $is_failure ) : ?>
			<div class="row outer_block">
				<div class="col-sm-8 col-sm-offset-2">
					<div class="alert alert-danger" role="alert">
						<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <strong><?php esc_html_e( 'Failure', 'bluff-post' ) ?></strong>
		...
		<table class="table">
			...
			<?php foreach ( $logs as $log ) : ?>
				...
				<tr>
					...
					<td>
						<span class="label label-<?php echo $level_labels[ $log->level ] ?>"><?php echo esc_html( $log->get_level_name() ) ?></span>
					</td>
					<td>
	...
	<form method="post" id="form_duplicate" class="form-inline" action="<?php echo admin_url( 'admin.php?page=blfpst-send-mail' ) ?>">
		<div class="row">
			...
			<div class="col-sm-7 text-right">
				<?php if ( $is_history ) : ?>
					<button type="submit" class="btn btn-default">
						<i class="fa fa-copy" aria-hidden="true"></i> <?php esc_html_e( 'Copy to create', 'bluff-post' ) ?>
					</button>
					<button type="button" class="btn btn-default"
					        onclick="registerTemplate(<?php echo esc_html( $send_mail_id ) ?>)">
					<i class="fa fa-plus" aria-hidden="true"></i> <?php esc_html_e( 'Registration template', 'bluff-post' ) ?>
					</button>
				<?php endif ?>
				...
				<?php if ( $is_reserved ) : ?>
					<button type="submit" class="btn btn-primary"
						...
						<span class="glyphicon glyphicon-edit" aria-hidden="true"></span> <?php esc_html_e( 'Change reservation', 'bluff-post' ) ?>
					</button>
				<?php endif ?>
